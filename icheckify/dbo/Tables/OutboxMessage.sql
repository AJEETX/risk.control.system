CREATE TABLE [dbo].[OutboxMessage] (
    ...
    CONSTRAINT [PK_OutboxMessage] PRIMARY KEY CLUSTERED ([OutboxMessageId] ASC),
    CONSTRAINT [FK_OutboxMessage_Mailbox_MailboxId] FOREIGN KEY ([MailboxId]) REFERENCES [dbo].[Mailbox] ([MailboxId]) ON DELETE CASCADE
);


GO
CREATE NONCLUSTERED INDEX [IX_OutboxMessage_MailboxId]
    ON [dbo].[OutboxMessage]([MailboxId] ASC);
