...
namespace risk.control.system.Services.AgencyAdmin
{
    ...
    internal class AgencyQueryReplyService(ApplicationDbContext context, ILogger<AgencyQueryReplyService> logger, ITimelineService timelineService) : IAgencyQueryReplyService
    {
        ...
        public async Task<bool> SubmitQueryReplyToCompany(string userEmail, long caseId, EnquiryRequest request, List<EnquiryRequest> requests, IFormFile? document)
        {
            try
            {
                var caseTask = await context.Investigations.Include(c => c.PolicyDetail).Include(p => p.ClientCompany).Include(c => c.InvestigationReport).ThenInclude(c => c!.EnquiryRequest).Include(c => c.InvestigationReport).ThenInclude(c => c!.EnquiryRequests).Include(c => c.Vendor)
                .FirstOrDefaultAsync(c => c.Id == caseId);
                const string replyByAgency = CONSTANTS.CASE_STATUS.CASE_SUBSTATUS.REPLY_TO_ASSESSOR;
                ...
                {
                    var dbEnquiry = caseTask.InvestigationReport.EnquiryRequests.FirstOrDefault(e => e.QueryRequestId == enquiry.QueryRequestId);
                    if (dbEnquiry != null)
                    {
