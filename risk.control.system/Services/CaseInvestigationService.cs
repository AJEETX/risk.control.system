...
namespace risk.control.system.Services
{
    ...
    {
        Task AssignToAssigner(string userEmail, List<long> claimsInvestigations);

        Task<CaseVerification> AllocateToVendor(string userEmail, long claimsInvestigationId, long vendorId, bool AutoAllocated = true);

        Task<CaseVerification> AssignToVendorAgent(string vendorAgentEmail, string currentUser, long vendorId, long claimsInvestigationId, string drivingMap, string drivingDistance, string drivingDuration, string distanceInMeters, string durationInSeconds);
        ...
        Task<CaseVerification> ProcessAgentReport(string userEmail, string supervisorRemarks, long claimsInvestigationId, SupervisorRemarkType remarks, IFormFile? claimDocument = null, string editRemarks = "");

        Task<(ClientCompany, string)> ProcessCaseReport(string userEmail, string assessorRemarks, long claimsInvestigationId, AssessorRemarkType assessorRemarkType, string reportAiSummary);

        List<VendorCaseModel> GetAgencyLoad(List<Vendor> existingVendors);

        Task<Vendor> WithdrawCase(string userEmail, ClaimTransactionModel model, long claimId);

        Task<List<long>> ProcessAutoAllocation(List<long> claims, ClientCompany company, string userEmail);
        Task<ClientCompany> WithdrawCaseByCompany(string userEmail, ClaimTransactionModel model, long claimId);
        Task<bool> SubmitNotes(string userEmail, long claimId, string notes);

        Task<CaseVerification> SubmitQueryToAgency(string userEmail, long claimId, EnquiryRequest request, IFormFile messageDocument);
        Task<CaseVerification> SubmitQueryReplyToCompany(string userEmail, long claimId, EnquiryRequest request, IFormFile messageDocument, List<string> flexRadioDefault);
    }
    ...
    public class CaseInvestigationService : ICaseInvestigationService
    {
        ...
        public async Task<List<long>> ProcessAutoAllocation(List<long> claims, ClientCompany company, string userEmail)
        {
            var autoAllocatedClaims = new List<long>();
            foreach (var claim in claims)
            {
                ...
                var claimsInvestigation = _context.CaseVerification
                    ...
                    .First(c => c.CaseVerificationId == claim);

                string pinCode2Verify = claimsInvestigation.CustomerDetail?.PinCode?.Code;
                ...
                if (vendorsInPincode is not null && vendorsInPincode.Count > 0)
                {
                    ...
                    if (vendorsWithCaseLoad is not null && vendorsWithCaseLoad.Count > 0)
                    {
                        ...
                        var policy = await AllocateToVendor(userEmail, claimsInvestigation.CaseVerificationId, selectedVendor.Vendor.VendorId);

                        autoAllocatedClaims.Add(claim);

                        await mailboxService.NotifyClaimAllocationToVendor(userEmail, policy.PolicyDetail.ContractNumber, claimsInvestigation.CaseVerificationId, selectedVendor.Vendor.VendorId);
                    }
                }
            }
        ...
        public async Task AssignToAssigner(string userEmail, List<long> claims)
        {
            if (claims is not null && claims.Count > 0)
            {
                var cases2Assign = _context.CaseVerification
                    .Where(v => claims.Contains(v.CaseVerificationId));
                ...
                foreach (var claimsInvestigation in cases2Assign)
                {
                    ...
                    var lastLog = _context.CaseVerificationTransaction
                        .Where(i =>
                            i.CaseVerificationId == claimsInvestigation.CaseVerificationId)
                            .OrderByDescending(o => o.Created)?.FirstOrDefault();

                    var lastLogHop = _context.CaseVerificationTransaction
                        .Where(i => i.CaseVerificationId == claimsInvestigation.CaseVerificationId)
                    ...
                    var log = new CaseVerificationTransaction
                    {
                        ...
                        CaseVerificationId = claimsInvestigation.CaseVerificationId,
        ...
        public async Task<ClientCompany> WithdrawCaseByCompany(string userEmail, ClaimTransactionModel model, long claimId)
        {
            var currentUser = _context.ClientCompanyApplicationUser.FirstOrDefault(u => u.Email == userEmail);
            var claimsInvestigation = _context.CaseVerification
                .FirstOrDefault(c => c.CaseVerificationId == claimId);
        ...
        public async Task<Vendor> WithdrawCase(string userEmail, ClaimTransactionModel model, long claimId)
        {
        ...
        public async Task<CaseVerification> AllocateToVendor(string userEmail, long claimsInvestigationId, long vendorId, bool AutoAllocated = true)
        {
            ...
            if (vendor != null)
            {
                var claimsCaseToAllocateToVendor = _context.CaseVerification
                    .Include(c => c.PolicyDetail)
                ...
                _context.CaseVerification.Update(claimsCaseToAllocateToVendor);
                var lastLog = _context.CaseVerificationTransaction.Where(i =>
                i.CaseVerificationId == claimsCaseToAllocateToVendor.CaseVerificationId).OrderByDescending(o => o.Created)?.FirstOrDefault();

                var lastLogHop = _context.CaseVerificationTransaction
                        .Where(i => i.CaseVerificationId == claimsInvestigationId)
                        .AsNoTracking().Max(s => s.HopCount);
                ...
                var log = new CaseVerificationTransaction
                {
                    ...
                    UserEmailActionedTo = "",
                    HopCount = lastLogHop + 1,
                    CaseVerificationId = claimsCaseToAllocateToVendor.CaseVerificationId,
        ...
        public async Task<CaseVerification> AssignToVendorAgent(string vendorAgentEmail, string currentUser, long vendorId, long claimsInvestigationId, string drivingMap, string drivingDistance, 
            string drivingDuration, string distanceInMeters, string durationInSeconds)
        {
            ...
            var assignedToAgent = _context.InvestigationCaseSubStatus.FirstOrDefault(
                        i => i.Name.ToUpper() == CONSTANTS.CASE_STATUS.CASE_SUBSTATUS.ASSIGNED_TO_AGENT);
            var claim = _context.CaseVerification
                .Include(c=>c.PolicyDetail)
                .Where(c => c.CaseVerificationId == claimsInvestigationId).FirstOrDefault();
            if (claim != null)
            {
                ...
                var lastLog = _context.CaseVerificationTransaction.Where(i =>
                i.CaseVerificationId == claim.CaseVerificationId).OrderByDescending(o => o.Created)?.FirstOrDefault();

                var lastLogHop = _context.CaseVerificationTransaction
                                        .Where(i => i.CaseVerificationId == claim.CaseVerificationId)
                                        .AsNoTracking().Max(s => s.HopCount);
                string timeElapsed = GetTimeElaspedFromLog(lastLog);
                ...
                var log = new CaseVerificationTransaction
                {
                    ...
                    HopCount = lastLogHop + 1,
                    CaseVerificationId = claim.CaseVerificationId,
            ...
            _context.CaseVerification.Update(claim);
            var rows = await _context.SaveChangesAsync();
            return claim;
        }
        ...
        public async Task<(Vendor, string)> SubmitToVendorSupervisor(string userEmail, long claimsInvestigationId, string remarks, string? answer1, string? answer2, string? answer3, string? answer4)
        {
            ...
            var claim = _context.CaseVerification
                ...
                .FirstOrDefault(c => c.CaseVerificationId == claimsInvestigationId);
            ...
            var lastLog = _context.CaseVerificationTransaction.Where(i =>
               i.CaseVerificationId == claimsInvestigationId).OrderByDescending(o => o.Created)?.FirstOrDefault();
            ...
            var log = new CaseVerificationTransaction
            {
                CaseVerificationId = claimsInvestigationId,
        ...
        public async Task<CaseVerification> ProcessAgentReport(string userEmail, string supervisorRemarks, long claimsInvestigationId, SupervisorRemarkType reportUpdateStatus, IFormFile? claimDocument = null, string editRemarks = "")
        {
        ...
        private async Task<(ClientCompany, string)> RejectCaseReport(string userEmail, string assessorRemarks, long claimsInvestigationId, AssessorRemarkType assessorRemarkType, string reportAiSummary)
        {
            ...
            try
            {
               ...
               .ThenInclude(c => c.CaseEnabler)
                ...
                var finalHop = _context.CaseVerificationTransaction
                                   .Where(i => i.CaseVerificationId == claimsInvestigationId)
                                    .AsNoTracking().Max(s => s.HopCount);
                var lastLog = _context.CaseVerificationTransaction.Where(i =>
              i.CaseVerificationId == claimsInvestigationId).OrderByDescending(o => o.Created)?.FirstOrDefault();

                var finalLog = new CaseVerificationTransaction
                {
                    HopCount = finalHop + 1,
                    UserEmailActioned = userEmail,
                ...
                _context.CaseVerificationTransaction.Add(finalLog);
                ...
                var invoice = new VendorInvoice
                {
                    ...
                    ClaimId = claimsInvestigationId.ToString()
                };
        ...
        private async Task<(ClientCompany, string)> ReAssignToCreator(string userEmail, long claimsInvestigationId, string assessorRemarks, AssessorRemarkType assessorRemarkType, string reportAiSummary)
        {
            var currentUser = _context.ClientCompanyApplicationUser.Include(c => c.ClientCompany).FirstOrDefault(u => u.Email == userEmail);

            var claimsCaseToReassign = _context.CaseVerification
                .Include(c => c.AgencyReport)
            ...
            _context.CaseVerification.Update(claimsCaseToReassign);
            var lastLog = _context.CaseVerificationTransaction.Where(i =>
                            i.CaseVerificationId == claimsCaseToReassign.CaseVerificationId).OrderByDescending(o => o.Created)?.FirstOrDefault();
            ...
            var log = new CaseVerificationTransaction
            {
                ...
                UserEmailActioned = userEmail,
                UserRoleActionedTo = $"{currentUser.ClientCompany.Email}",
                CaseVerificationId = claimsCaseToReassign.CaseVerificationId,
                Created = DateTime.Now,
                Time2Update = DateTime.Now.Subtract(lastLog.Created).Days,
        ...
        private async Task<CaseVerification> ApproveAgentReport(string userEmail, long claimsInvestigationId,  string supervisorRemarks, SupervisorRemarkType reportUpdateStatus, IFormFile? claimDocument = null, string editRemarks = "")
        {
            var claim = _context.CaseVerification
                .Include(c => c.PolicyDetail)
                .Include(c => c.AgencyReport)
        ...
        private async Task<CaseVerification> ReAllocateToVendorAgent(string userEmail, long claimsInvestigationId, string supervisorRemarks, SupervisorRemarkType reportUpdateStatus)
        {
        ...
        public async Task<CaseVerification> SubmitQueryToAgency(string userEmail, long claimId, EnquiryRequest request, IFormFile messageDocument)
        {
            var claim = _context.CaseVerification
                .Include(c => c.AgencyReport)
                .Include(c => c.Vendor)
        ...
        public async Task<CaseVerification> SubmitQueryReplyToCompany(string userEmail, long claimId, EnquiryRequest request, IFormFile messageDocument, List<string> flexRadioDefault)
        {
            var claim = _context.CaseVerification
                .Include(c => c.PolicyDetail)
                .Include(p => p.ClientCompany)
        ...
        private string GetTimeElaspedFromLog(CaseVerificationTransaction lastLog)
        {
