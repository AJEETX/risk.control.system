...
namespace risk.control.system.Services.Api
{
    ...
    internal class AgencyService : IAgencyService
    {
        ...
        public async Task<List<AgencyServiceResponse>> GetAgencyService(long vendorId)
        {
            ...
            var services = await _context.VendorInvestigationServiceType
        ...
        public async Task<ConcurrentBag<AgentData>> GetAgentWithCases(string userEmail, long caseId)
        {
            ...
            // 1️⃣ Pre-fetch vendor agents with only needed includes
            var vendorAgentsQuery = _context.ApplicationUser
                .Where(u => u.VendorId == vendorUser.VendorId
                            && !u.Deleted
                            && u.Active
                            && u.Role == AppRoles.AGENT);

            if (onboardingEnabled)
                vendorAgentsQuery = vendorAgentsQuery.Where(u => !string.IsNullOrWhiteSpace(u.MobileUId));
            ...
            string locationLat = caseTask.PolicyDetail.InsuranceType == InsuranceType.UNDERWRITING
                ? caseTask.CustomerDetail.Latitude
                : caseTask.BeneficiaryDetail.Latitude;

            string locationLng = caseTask.PolicyDetail.InsuranceType == InsuranceType.UNDERWRITING
                ? caseTask.CustomerDetail.Longitude
                : caseTask.BeneficiaryDetail.Longitude;

            var agentList = new ConcurrentBag<AgentData>();

            // 4️⃣ Pre-load photos asynchronously
            var photoTasks = vendorAgents.Select(agent =>
                agent.ProfilePictureUrl != null
                    ? File.ReadAllBytesAsync(Path.Combine(env.ContentRootPath, agent.ProfilePictureUrl))
                    : Task.FromResult(Array.Empty<byte>())
            ).ToArray();

            var photoResults = await Task.WhenAll(photoTasks);

            // 5️⃣ Parallelize processing agents
            var mapTasks = vendorAgents.Select(async (agent, idx) =>
            {
