...
namespace risk.control.system.Services.Creator
{
    ...
    internal class CaseActiveService : ICaseActiveService
    {
        ...
        {
            var caseTask = await GetActiveCaseDetail(id);
            ...
            return new CaseTransactionModel
            {
                ...
                Culture = CustomExtensions.GetCultureByCountry(companyUser!.ClientCompany!.Country!.Code.ToUpper())

            };
        }
        private async Task<InvestigationTask?> GetActiveCaseDetail(long caseId)
        {
