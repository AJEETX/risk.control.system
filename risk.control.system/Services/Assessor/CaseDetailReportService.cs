...
namespace risk.control.system.Services.Assessor
{
    ...
    internal class CaseDetailReportService : ICaseDetailReportService
    {
        ...
        {
            var caseTask = await GetClaimDetailsReport(id);
            ...
            var totalTimeTaken = timeTaken != TimeSpan.Zero ? $"{(timeTaken.Days > 0 ? $"{timeTaken.Days}d " : "")}" + $"{(timeTaken.Hours > 0 ? $"{timeTaken.Hours}h " : "")}" + $"{(timeTaken.Minutes > 0 ? $"{timeTaken.Minutes}m " : "")}" + $"{(timeTaken.Seconds > 0 ? $"{timeTaken.Seconds}s" : "less than a sec")}" : "-";
            ...
            var templates = await GetReportTemplate(caseTask.ReportTemplateId);
        ...
        private async Task<ReportTemplate?> GetReportTemplate(long? taskId)
        {
            return await _context.ReportTemplates.AsNoTracking()
              ...
              .Include(r => r.LocationReport)
                  ...
                  .FirstOrDefaultAsync(q => q.Id == taskId);
        }
        private async Task<InvestigationTask?> GetClaimDetailsReport(long id)
        {
        ...
        {
            var caseTask = await GetInvestigateReport(selectedcase);
            if (caseTask is null) return null!;
            var beneficiaryDetails = await _context.BeneficiaryDetail.AsNoTracking().Include(c => c.PinCode).Include(c => c.BeneficiaryRelation).Include(c => c.District).Include(c => c.Country).Include(c => c.State).FirstOrDefaultAsync(c => c.BeneficiaryDetailId == caseTask.BeneficiaryDetail!.BeneficiaryDetailId);
            caseTask.CustomerDetail!.PhoneNumber = new string('*', caseTask.CustomerDetail!.PhoneNumber.ToString().Length - 4) + caseTask.CustomerDetail.PhoneNumber.ToString().Substring(caseTask.CustomerDetail.PhoneNumber.ToString().Length - 4);
        ...
        private async Task<InvestigationTask?> GetInvestigateReport(long? selectedcase)
        {
            return await _context.Investigations.AsNoTracking()
               .Include(c => c.InvestigationTimeline)
               .Include(c => c.InvestigationReport)
