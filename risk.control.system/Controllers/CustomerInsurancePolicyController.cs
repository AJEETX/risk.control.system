...
namespace risk.control.system.Controllers
{
    public class CustomerInsurancePolicyController : Controller
    {
        ...
        [Breadcrumb(title: " Add Customer", FromAction = "IndexAuto", FromController = typeof(InsuranceClaimsController))]
        public async Task<IActionResult> CreateCustomerAuto(string id)
        {
            try
            {
                if (id == null || string.IsNullOrWhiteSpace(id))
        ...
        [Breadcrumb(title: " Add Customer", FromAction = "IndexManual", FromController = typeof(InsuranceClaimsController))]
        public async Task<IActionResult> CreateCustomerManual(string id)
        {
            try
            {
                if (id == null || string.IsNullOrWhiteSpace(id))
                ...
                var editPage = new MvcBreadcrumbNode("CreateCustomerManual", "CustomerInsurancePolicy", $"Add Customer") { Parent = detailsPage, RouteValues = new { id = id } };
                ViewData["BreadcrumbNode"] = editPage;
