...
namespace risk.control.system.Controllers.Company
{
    ...
    public class EmpanelledAgencyController : Controller
    {
        ...
        public async Task<IActionResult> EmpanelledVendors(List<long> vendors)
        {
            try
            {
                if (!ModelState.IsValid || vendors is null || vendors.Count == 0)
                {
                    ...
                    return RedirectToAction(nameof(EmpanelledVendors), "EmpanelledAgency");
                }
        ...
        [Breadcrumb("Agency Profile", FromAction = "EmpanelledVendors")]
        ...
        public async Task<IActionResult> EditAgency(long id)
        {
            try
            {
                ...
                var editPage = new MvcBreadcrumbNode("EditAgency", "EmpanelledAgency", $"Edit Agency") { Parent = detailsPage, RouteValues = new { id = id } };
        ...
        public async Task<IActionResult> EditAgency(long vendorId, Vendor model)
        {
            ...
            {
                logger.LogError(ex, "Error editing {AgencyId} for {UserEmail}.", vendorId, HttpContext.User?.Identity?.Name);
                notifyService.Error("Error editing agency. Try again.");
            }
            return RedirectToAction(nameof(AgencyDetail), "EmpanelledAgency", new { id = vendorId });
        }
    }
}