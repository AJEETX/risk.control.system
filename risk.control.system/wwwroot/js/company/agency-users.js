    ...
    var table = $("#dataTable").DataTable({
        ajax: {
            ...
            error: function (xhr, status, error) {
                ...
                if (xhr.status === 500) {
                    $.confirm({
                        title: 'Server Error!',
                        content: 'An unexpected server error occurred. You will be redirected to Agency User page.',
                        type: 'orange',
                        typeAnimated: true,
                        buttons: {
                            Ok: function () {
                                window.location.href = '/EmpanelledAgencyUser/Users?id=' + $('#Id').val(); // Server error. Try again
                            }
                        },
                        onClose: function () {
                            window.location.href = '/EmpanelledAgencyUser/Users?id=' + $('#Id').val(); // Server error. Try again
        ...
        columns: [
            ...
            },
            {
                ...
                "mRender": function (data, type, row) {
                    var buttons = "";
                    buttons += '<a id="edit' + row.id + '" href="/EmpanelledAgencyUser/EditAgencyUser?userId=' + row.id + '" class="btn btn-xs btn-warning"><i class="fas fa-edit"></i> Edit</a>&nbsp;'
