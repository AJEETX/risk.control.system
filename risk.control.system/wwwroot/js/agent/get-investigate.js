...
$(document).ready(function () {
    ...
    $(".upload-face-btn").click(function () {
        ...
        var isAgent = $(this).data("isagent") === true || $(this).data("isagent") === "true";
        ...
        $.ajax({
            ...
            success: function (response) {
                $("#upload-status-" + faceId).text("Upload successful!");
            },
            error: function () {
                $("#upload-status-" + faceId).text("Upload failed. Try again.");
            }
        });
    });
    ...
    $(".upload-doc-btn").click(function () {
        ...
        $.ajax({
            ...
            contentType: false,
            success: function () {
                $("#doc-upload-status-" + docId).text("Upload successful!");
                var img = $("#doc-img-" + docId);
                var currentSrc = img.attr("src").split('?')[0];
                img.attr("src", currentSrc + "?t=" + new Date().getTime());
            },
            error: function () {
                $("#doc-upload-status-" + docId).text("Upload failed. Try again.");
            }
        });
    });
